...
// Cada una con fecha de finalización real (domingo 23:59 UTC)
// Límite de 50 cupos por semana — tracking real desde la DB
...
const promotions = [
  ...
  { id: 18, type: 'commission_boost', title: 'As en la Manga', desc: '42% en Plan Pro. Mejor semana para vender Pro', value: 42, bonus: 17, color: 'from-indigo-600 to-blue-600' },
  ...
  { id: 72, type: 'double_plan', title: 'Upgrade Week', desc: '50% si referís upgrades (Starter→Pro, Pro→AI)', value: 50, bonus: 25, color: 'from-teal-500 to-green-500' },
  ...
  { id: 93, type: 'special', title: 'San Valentín', desc: 'Referí a un colega: 45% comisión para los dos', value: 45, bonus: 20, color: 'from-pink-500 to-red-500' },
...
const MAX_SLOTS_PER_WEEK = 50

/**
 * Devuelve el timestamp (ms) del final de la semana actual (domingo 23:59:59 UTC)
 */
function getWeekEnd() {
  const now = new Date()
  const day = now.getUTCDay() // 0 = Sunday
  const daysUntilSunday = day === 0 ? 0 : 7 - day
  const end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + daysUntilSunday, 23, 59, 59, 999))
  return end.getTime()
}
...
export function getCurrentPromotion() {
  ...
  return {
    ...
    week,
    endsAt: getWeekEnd(),
    maxSlots: MAX_SLOTS_PER_WEEK,
  }
}

export function getPromotionByWeek(week) {
  const index = (week - 1) % promotions.length
  return { ...promotions[index], week, endsAt: getWeekEnd(), maxSlots: MAX_SLOTS_PER_WEEK }
}
