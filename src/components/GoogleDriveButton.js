...
export default function GoogleDriveButton({ content, filename = 'documento.txt', mimeType = 'text/plain' }) {
  ...
  async function checkConnection() {
    try {
      ...
      if (user) {
        setUserId(user.id)
        setConnected(!!user.user_metadata?.drive_access_token)
      }
    } catch {}
  }
  ...
  async function connectDrive() {
    if (!userId) {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return toast.error('Iniciá sesión primero')
      setUserId(user.id)
    }

    const res = await fetch(
      `/api/google-drive/auth-url?userId=${userId}&returnTo=${encodeURIComponent(window.location.pathname)}`
    )
    const { url, error } = await res.json()
    if (error) {
      if (error.includes('not configured')) {
        return toast.error('Google Drive no configurado. El administrador debe agregar GOOGLE_CLIENT_ID en Vercel.')
      }
